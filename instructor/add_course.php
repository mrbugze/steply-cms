...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if (isset($_POST['steps']) && is_array($_POST['steps'])) {
            foreach ($_POST['steps'] as $step_key => $step_data) {
                ...
                if (empty($step_title) || empty($step_content)) {
                    // Optionally skip empty steps or throw an error
                    continue; 
                }
                ...
                if (isset($step_data['tasks']) && is_array($step_data['tasks'])) {
                    foreach ($step_data['tasks'] as $task_key => $task_data) {
                        $task_title = trim($task_data['title'] ?? '');
                        $task_type = trim($task_data['type'] ?? 'assignment'); // Default type
                        $task_content = trim($task_data['content'] ?? '');

                        if (empty($task_title)) {
                            // Optionally skip empty tasks or throw an error
                            continue; 
                        }
        ...
        $_SESSION['message'] = "Course, steps, and tasks added successfully.";
...
<div class="container mt-4">
    ...
    <form action="add_course.php" method="post" id="addCourseForm" class="form-container card p-4 shadow-sm">
        <fieldset class="mb-4" style="
    padding: 26px;">
