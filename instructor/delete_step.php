...
// Check if the step exists and belongs to a course owned by the instructor
$stmtCheck = $conn->prepare("SELECT s.step_id, s.course_id 
                        FROM steps s 
...
if (!$stepData) {
    ...
    header("Location: " . $redirect_url . "?error=Step not found or permission denied.");
    exit;
}
